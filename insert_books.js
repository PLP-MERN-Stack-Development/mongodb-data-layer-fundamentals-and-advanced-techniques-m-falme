...
async function insertBooks() {
  ...
  try {
    ...
    // Optional: clear existing data so we start clean each run
    const existing = await col.countDocuments();
    if (existing > 0) {
      console.log(`⚠️  Collection already has ${existing} docs — deleting them for a clean insert.`);
      await col.deleteMany({});
    }

    const res = await col.insertMany(books);
    console.log(`📚 Inserted ${res.insertedCount} books into ${dbName}.${collectionName}`);

    // Print a sample of inserted documents
    const sample = await col.find({}).limit(5).toArray();
    console.log('\nSample inserted documents:');
    sample.forEach((b, i) => console.log(`${i + 1}. ${b.title} — ${b.author} (${b.published_year})`));
  } catch (err) {
